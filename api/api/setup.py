...
def load_config(app):
    ...
    for email in config.get('debug', 'admin_emails').split(','):
        log.debug(" Adding '%s' to admin_emails", email)
    ...
    if secret_key == '':
        log.critical('The Flask secret key specified in the config file is empty.')
        exit()
    ...
    if write_logs_to_db:
        log.debug("Enabling 'write logs to database'")
